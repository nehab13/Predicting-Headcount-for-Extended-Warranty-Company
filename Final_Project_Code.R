...
######################################################################
#Using K-NN with time series to Predict Calls

setwd("D:/MSBA School Work/Fall 2021/Capstone Project")

set.seed(111)
final1.data <- read.csv("Call_HT_data_new.csv")
head(Call_HT.data)

rel = cor.test(final1.data$No.of.Calls, final1.data$Claims, method = c("kendall"))

rel$estimate

#This shows us that the correlation between the No. of Calls and No. of Claims is not very strong
#This is why we are not going to use any relationship models.
#Since we arleady know that the time series performs better we are going to use that
#Now we must construct a K-NN model with time series to see if it performs better.

library(tsfknn)
knn.df <- final1.data$No.of.Calls
knn.df
train2.df = final1.data$No.of.Calls[-c(52:57)]
summary(train2.df)
pred.knn <- knn_forecasting(ts(train2.df), h = 6, lags = 1:4, k = 6, transform = "additive")
summary(pred.knn)
knn.pred = pred.knn$prediction
knn.pred
valid2.data = final1.data$No.of.Calls[c(52:57)]
valid2.data
round(accuracy(knn.pred, valid2.data), 3)

#As we can see here the accuracy metrics show our RMSE and MAPE both being better with this model
#So this very basic yet powerful machine learning K-NN model gives us the best accuracy
#Now let's predict the next six months
final.df <- final1.data$No.of.Calls
final.df
pred.final <- knn_forecasting(ts(final.df), h = 6, lags = 1:6, k = 4, transform = "additive")
summary(pred.final)
#pred.final$prediction
#knn_examples(pred.final)
#Since we have our validation data in our dataset we can back-test our data to see how our model will perform 
#Here I will use the rolling origin function, ro(), to achieve that 
ro <- rolling_origin(pred.final, h = 6)
print(ro$global_accu)
plot(ro, h=6)

print(ro$test_sets)
print(ro$errors)
final.pred = pred.final$prediction

plot(pred.final)
print(ro$predictions)
final.pred      

#As we can see that our global accuracy is under 4% MAPE value and an RMSE of 5405.
#This means that our current model is able to predict the number of calls with less than 5% error
